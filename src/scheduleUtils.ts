...
function isNearDuplicate(hex: string, colours: ColourEntry[], threshold = 30) {
	...
	return colours.some((c) => colorDistance(target, hexToRgb(c.colour)) < threshold);
}
...
export function validatePreferences(form: PreferencesForm): FormErrors<PreferencesForm> {
...
export function loadSchedule(): Schedule | null {
	try {
		...
		// Basic sanity check before trusting the stored value
